...
- [x] 3.1 [ROJO] Ajustar fila `GameDetail (ficha sin emojis)` sobre `src/Ludeka.Web/Components/Pages/GameDetail.razor`: mustContain + `flex-wrap`, `text-[var(--on-brand)]`; mustNotContain + `Ludeca`, `text-white`, `text-slate-400`, `bg-amber-500`, `bg-indigo-500`, `bg-purple-950`, `bg-rose-500`, `text-orange-400`. La fila `GameDetail (diseñador texto plano)` (read-only) no cambia: se protege en verde. Rojo: falla.
- [x] 3.2 [VERDE] En `src/Ludeka.Web/Components/Pages/GameDetail.razor`: `<PageTitle>` «Ludeka» (corrige errata «Ludeca»); estado «juego no encontrado» tokenizado (líneas 16/25/26/27: mapa DD-04 — `text-white` invisible → `--text-primary`); back-bar DD-08 (líneas 38-108: `flex flex-wrap`, moderación agrupada en subcontenedor con borde, «Ir a Mi Ludoteca» al grupo izquierdo de navegación); hardcodes → tokens (56 hover rose → `--state-error-*`, 76 → `--state-warning-*`, 83/294 → `--state-info-*`, 131 chip sólido → `bg-[var(--state-highlight)] text-[var(--on-brand)]`, 299 → `--text-muted`/`--text-primary`); botón «Generar Síntesis con IA Ahora» (323) → `text-[var(--on-brand)]`. Verde: 3.1 + diseñador intacto. Commit: `feat: ficha editorial tokenizada con back-bar envolvente`.
- [x] 3.3 [Boundary PR-3] `dotnet test Ludeka.sln` verde (incluye diseñador texto plano); smoke ficha de ejemplo: con y sin moderador (la fila envuelve en móvil sin desbordar) y estado no-encontrado legible en los 5 temas. Pushear y abrir PR-3 (base = rama del PR-2).
