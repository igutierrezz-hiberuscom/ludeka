...
#### Escenario: Actualizar entidad Game existente
- DADO un juego existente con cambios en `LudistRating` o metadatos
- CUANDO se invoca `UpdateAsync(game, cancellationToken)`
- ENTONCES los cambios DEBEN quedar persistidos en el almacenamiento subyacente.
...
### Requerimiento: Bloque Editorial en Ficha Inteligente y Botón de Gestión Contextual
La página de detalle del juego DEBE renderizar en su zona superior el bloque editorial correspondiente (`FoundingVerdictCard` o `AiSummaryCard`) y el botón `[ 🛡️ Gestionar Veredicto Fundador ]` visible exclusivamente para moderadores.

#### Escenario: Renderizado del veredicto fundador en cabecera
- DADO un juego con veredicto emitido por la mesa fundadora
- CUANDO cualquier usuario visita `/juegos/{slug}`
- ENTONCES la tarjeta del veredicto oficial con su sello, análisis y fotos DEBE posicionarse de manera fija y prioritaria sobre las opiniones públicas.

#### Escenario: Acceso editorial condicional por rol
